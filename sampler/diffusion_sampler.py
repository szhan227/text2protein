...
class DiffusionSampler(torch.nn.Module):
    ...
    def ddim_sample(self, shape, cond, clip_scheme='static'):
        '''
        ...
        for t, t_next in tqdm(time_pairs, desc='DDIM sampling loop time step'):
            ...
            pred_noise, x_start = self.model_predictions(output, cond, time_cond, clip_scheme=clip_scheme)
    ...
    def model_predictions(self, x, cond, time_cond, clip_scheme='static'):
        '''
        :param x: input image
        :param cond: conditioning, here our textual description embedding
        ...
        if cond is None:
            model_output = self.model(x, cond, time_cond)
        else:
            model_output = self.w * self.model(x, cond, time_cond) + (1 - self.w) * self.model(x, cond * 0, time_cond)
